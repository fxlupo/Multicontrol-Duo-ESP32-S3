...
bool events::loadRecentFromBackend(uint8_t limit) {
    ...
    HTTPClient http;
    http.begin(apiClient(), String(API_BASE_URL) + "/events?limit=" + String(limit));
    addAuth(http);
    http.setTimeout(HTTP_TIMEOUT_MS);
    int code = http.GET();
    if (code != 200) {
...
void events::postStatus() {
    ...
    doc["firmwareVersion"] = "2.2.7";
    doc["ipAddress"]       = WiFi.localIP().toString();
    String body;
    serializeJson(doc, body);
    postJson("/status", body);
}
